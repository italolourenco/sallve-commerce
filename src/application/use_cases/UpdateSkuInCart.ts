...
export class UpdateSkuInCart {
  ...
  async execute(cartId: number, skuId: number, quantity: number) {
    try {
      ...
      const newInventoryValue = quantity;
      const checkNewInventoryValueIsValid =
        sku.inventory + quantityInCart - quantity;
      ...
      return { statusCode: 200, message: "Sku Update Successfully" };
    } catch (error) {
